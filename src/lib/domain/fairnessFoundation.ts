 ...
 *   it, so every member's personal target defaults to 0 instead of a real
 *   proportional share. This is deliberately NOT raised merely because
 ...
 *   computable.
 * - `shift_target_unmodelable_evidence_only` -- this person is a
 *   comparison-group member ONLY because of real confirmed shift evidence,
 *   not because their CURRENT capability flag matches the role (see
 *   `fairnessShiftEngine.ts`'s `isRoleComparisonMember`) -- capability
 *   isn't historically dated, so there is no honest way to reconstruct
 *   what opportunities they might have had at the time. Their real
 *   `actualShifts` stays visible, but their personal `target` is fixed at
 *   `0` rather than computed, and -- critically -- their workload is
