...
/**
 * Shared upsert-or-cancel application for both tomorrow-reminder
 * categories: every job whose content is still valid this tick is
 * upserted (creating it, or refreshing its content if the underlying
 * assignment changed before send -- spec sections 16-17); every
 * previously-created, still-pending job for the same date whose
 * dedupe_key is no longer among the valid set (the assignment
 * disappeared) is cancelled.
 */
async function applyReminderJobs(
  category: "tomorrow_shift" | "tomorrow_duty",
  tomorrowDate: string,
