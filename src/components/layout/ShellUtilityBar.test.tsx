...
describe("ShellUtilityBar", () => {
  ...
  it("renders both organizational logos at their calibrated, visually-substantial sizes (header-logo-correction pass) -- never shrunk back to icon scale", () => {
    render(<ShellUtilityBar initialClockTime="10:00:00" dateLabel={null} />);
    const takshal = screen.getByAltText('תקש"ל');
    const strategic = screen.getByAltText("תקשורת אסטרטגית");
    ...
    // emblems read as the same PERCEIVED size, not the same box. Grown
    // meaningfully from 49px/64px in the header-logo-correction pass (these
    // two org marks, not `BrandMark`, were the actual intended target),
    // while keeping the same ≈1.3 ratio between them.
    expect(takshal.className).toMatch(/h-\[90px\]/);
    expect(strategic.className).toMatch(/h-\[69px\]/);
  });
});
