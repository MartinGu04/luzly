 ...
 * UX pass, and corrected in a follow-up pass) -- the completed/target
 * progress summary (remaining or beyond-potential, plus pace) leads,
 * exactly like the main card, then a LIVE strip when relevant, then a
 * plain stat grid of the workbook's own record for this person (previous/
 * current score, the change since last period, weekend count).
 *
 ...
 * person's own published-potential total, shown above) is now the ONLY
 * user-facing duty target -- showing the role-wide constant alongside it,
 * under any label, would read as a second, competing target for the exact
 * same word ("יעד"/"target"), even when the two numbers legitimately
 * differ (e.g. a workbook comparisonTarget of 8 next to this person's own
 * personalTargetTotal of 5.4). `comparisonTarget`/`gapToTarget`/
 * `normalizedLoad`/`status` remain computed in the read model, unchanged,
 * because `compareDutyFairnessRows`'s existing row ordering still depends
 * on `normalizedLoad` -- this is a UI-only removal, not a calculation
 * change. `currentScore`/`previousScore`/`delta` stay visible here since
 * they are plain SCORE facts (the workbook's official record), never
 * framed as a target of any kind. Exemptions render as visible badges
 * directly in the panel, never behind hover.
 */
