 ...
 *
 * `critical` gets real visual presence: the `critical` surface variant
 * (subtle red-tinted background/border, never a flashing whole card), a
 * stronger heading, and a small `PulseIndicator` -- the SAME restrained,
 * reduced-motion-respecting pulse already used for "live now" elsewhere,
 * reused rather than a new bespoke animation. Motion is never the only
 * signal: the heading text, count, and icon color all still communicate
 * severity with animation off. `review`/`info` never receive any of this --
 * same neutral `panel` surface and heading weight as before.
 */
