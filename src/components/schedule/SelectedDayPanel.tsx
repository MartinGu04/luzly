...
/**
 * The selected calendar day's full detail section -- every personal shift
 * on that date, never collapsed to one even when there are several. Shows
 * only what the server already resolved (`timing`) -- never invents a time
 * for a shift whose hour can't be evaluated.
 */
export function SelectedDayPanel({ dayMeta, events }: SelectedDayPanelProps) {
  if (!dayMeta) return null;

  return (
    <Panel variant="panel" className="mt-4">
      <div className="flex flex-wrap items-center justify-between gap-x-3 gap-y-2">
        <p className="text-sm font-semibold text-foreground sm:text-base">{dayMeta.dateLabel}</p>
        {dayMeta.holiday ? (
